...
export async function humaniser(matiere, { secondes = 60, question = true } = {}) {
  ...
  const system =
    ...
    (question
      ? `6. TERMINE en deux temps, dans cet ordre : d'abord une QUESTION ouverte au spectateur qui donne\n` +
        `   envie de répondre en commentaire, puis UNE phrase courte invitant à liker, s'abonner et partager.\n` +
        `   Reste naturel : "Dites-le en commentaire. Et si ça vous a été utile, un like, un partage,\n` +
        `   et abonnez-vous pour la suite." — jamais de ton publicitaire.\n`
...
export async function humaniserJT(sujets, { motsParSujet = 55 } = {}) {
  ...
  const system =
    ...
    `- LONGUEUR DE CHAQUE PASSAGE : entre ${bas} et ${haut} mots. C'est impératif — un passage de deux\n` +
    `  lignes transforme le journal en liste de brèves. Développe : le fait, puis ce que ça change.\n` +
    ...
    `  puis une phrase courte et naturelle invitant à liker, s'abonner et partager. Jamais de ton publicitaire.\n` +
