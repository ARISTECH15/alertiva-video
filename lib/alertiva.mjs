...
/**
 * Photo LIBRE DE DROITS d'une personnalité publique via Wikipédia/Wikimedia Commons
 * (image principale de la fiche = fichier Commons, licence CC/domaine public pour les personnes).
 * Essaie FR puis EN. Renvoie l'URL (redimensionnée ~1080) ou null. Zéro risque de monétisation.
 */
export async function wikimediaImage(name) {
  ...
  for (const lang of ["fr", "en"]) {
    try {
      ...
      const url = `https://${lang}.wikipedia.org/w/api.php?action=query&format=json&redirects=1&prop=pageimages&piprop=thumbnail&pithumbsize=1080&titles=${encodeURIComponent(q)}&origin=*`;
      const r = await fetch(url, { headers: { "User-Agent": "AlertivaNews/1.0 (+https://alertivanews.com)" }, signal: ctrl.signal });
      clearTimeout(t);
      if (!r.ok) continue;
      const j = await r.json();
...
/**
 * Jusqu'à n CLIPS VIDÉO de banque (Pexels Videos, licence commerciale gratuite), portrait,
 * fichier MP4 pas trop lourd. Donne du mouvement (« images vivantes »). Renvoie des URLs.
 */
export async function stockVideos(query, n = 1) {
  const key = process.env.PEXELS_API_KEY || (await settingsValue("img_key_pexels"));
  if (!key || !query) return [];
  try {
    const ctrl = new AbortController();
    const t = setTimeout(() => ctrl.abort(), 15000);
    const r = await fetch(
      `https://api.pexels.com/videos/search?query=${encodeURIComponent(query)}&orientation=portrait&per_page=${Math.max(1, n)}&size=medium`,
    ...
    for (const v of j.videos || []) {
      const files = (v.video_files || []).filter((f) => f.file_type === "video/mp4" && (f.height || 0) >= (f.width || 0));
      files.sort((a, b) => (a.width || 9999) - (b.width || 9999));
      const pick = files.find((f) => (f.width || 0) >= 540) || files[0];
...
// ---------- TTS Qwen-Audio-3.0-TTS via OpenRouter (voix générée) ----------

/**
 * Synthèse vocale via Qwen-Audio-3.0-TTS (OpenRouter, API OpenAI-compatible `/audio/speech`).
 * Réutilise OPENROUTER_API_KEY. Écrit un MP3 dans outMp3Abs. LÈVE une erreur en cas d'échec
 * (l'appelant peut basculer sur ttsBest — ElevenLabs/edge-tts). Réponse gérée en binaire OU JSON.
 */
export async function ttsQwen(text, outMp3Abs, { voice, model, instructions } = {}) {
  const key = process.env.OPENROUTER_API_KEY;
  if (!key) throw new Error("OPENROUTER_API_KEY manquante (TTS Qwen).");
  const body = {
    model: model || process.env.TTS_MODEL || "qwen/qwen-audio-3.0-tts-plus",
    input: String(text || "").slice(0, 8000),
    voice: voice || process.env.TTS_VOICE || "longanlufeng",
    response_format: "mp3",
  };
  // Qwen-Audio-3.0 accepte une instruction de ton (rôle/émotion/débit) — on impose un ton "info".
  const instr = instructions || process.env.TTS_INSTRUCTIONS ||
    "Ton grave et sérieux de présentateur de journal télévisé, débit posé et autoritaire, articulation nette.";
  ...
  if (!r.ok) throw new Error(`TTS Qwen HTTP ${r.status} : ${(await r.text()).slice(0, 300)}`);
  let buf;
  if ((r.headers.get("content-type") || "").includes("application/json")) {
    const j = await r.json().catch(() => ({}));
    const b64 = j?.audio || j?.data?.[0]?.b64_json || j?.data?.[0]?.audio || null;
    const url = j?.url || j?.data?.[0]?.url || null;
    if (b64) buf = Buffer.from(String(b64).split(",").pop(), "base64");
    else if (url) buf = Buffer.from(await (await fetch(url)).arrayBuffer());
    else throw new Error("TTS Qwen : JSON sans audio : " + JSON.stringify(j).slice(0, 200));
  } else {
    buf = Buffer.from(await r.arrayBuffer());
  }
  if (!buf || buf.length < 1000) throw new Error("TTS Qwen : audio vide/trop court.");
  fs.writeFileSync(outMp3Abs, buf);
  return outMp3Abs;
}

/**
 * Copywriting réseaux : à partir du script d'une vidéo, renvoie {description, hashtags[]} via le
 * LLM (Groq, déjà configuré). Parsing JSON défensif. Renvoie null si indisponible.
 */
export async function copywriting(script) {
  const key = await groqKey();
  if (!key) return null;
  const system =
    `Tu es responsable social media d'un média d'actualité (Alertiva).\n` +
    `À partir du script d'une vidéo, produis un objet JSON EXACTEMENT de la forme :\n` +
    `{"description":"…","hashtags":["#…","#…"]}\n` +
    `- "description" : accroche percutante pour TikTok/Instagram/YouTube, en français, 1 à 2 phrases\n` +
    `  (max ~220 caractères), terminée par un appel à commenter. Pas d'emoji à outrance.\n` +
    `- "hashtags" : 5 à 8 hashtags pertinents en français, sans espaces, dont #alertiva et #actu.\n` +
    ...
    try {
      ...
      const r = await fetch("https://api.groq.com/openai/v1/chat/completions", {
        ...
        body: JSON.stringify({
          ...
          messages: [{ role: "system", content: system }, { role: "user", content: String(script || "").slice(0, 6000) }],
        }),
      });
      clearTimeout(t);
      const j = await r.json().catch(() => ({}));
      ...
      const description = stripMd(String(parsed.description || "")).trim();
      let hashtags = Array.isArray(parsed.hashtags) ? parsed.hashtags.map((h) => String(h).trim()).filter(Boolean) : [];
      hashtags = hashtags.map((h) => (h.startsWith("#") ? h : "#" + h).replace(/\s+/g, ""));
      if (description && hashtags.length >= 3) return { description, hashtags: hashtags.slice(0, 8) };
...
export async function cardStoryboard(matiere, n = 6) {
  ...
  const system =
    ...
    `- "entity" : le NOM COMPLET d'UNE personnalité publique réelle et identifiable au cœur de cette\n` +
    `  section (ex. "Gérald Darmanin"), ou "" si la section ne porte pas sur une personne précise ;\n` +
    `- "query" : 2 à 4 mots-clés EN ANGLAIS pour un clip vidéo de banque (scène/objet CONCRET, SANS nom propre) ;\n` +
    `- "image" : un prompt d'image EN ANGLAIS = photo d'actualité éditoriale verticale, réaliste, SANS texte ni logo.\n` +
    `N'invente AUCUN fait absent du texte. Réponds UNIQUEMENT avec ce JSON, rien d'autre :\n` +
    `{"sections":[{"headline":"…","summary":"…","entity":"…","query":"…","image":"…"}]} — exactement ${n} sections.`;
  const modeles = ["openai/gpt-oss-120b", "openai/gpt-oss-20b", "llama-3.3-70b-versatile"];
  for (const model of modeles) {
    try {
      const ctrl = new AbortController();
      ...
      const sections = (Array.isArray(parsed?.sections) ? parsed.sections : [])
        .filter((s) => s && s.headline && s.summary)
        .map((s) => ({
          headline: stripMd(String(s.headline)),
          summary: stripMd(String(s.summary)),
          entity: s.entity ? stripMd(String(s.entity)) : "",
          query: s.query ? String(s.query).replace(/["']/g, "").trim() : "",
