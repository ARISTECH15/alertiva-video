...
export async function ttsQwen(text, outMp3Abs, { voice, model, instructions } = {}) {
  ...
  const body = {
    model: model || process.env.TTS_MODEL || "x-ai/grok-voice-tts-1.0",
    input: String(text || "").slice(0, 8000),
    voice: voice || process.env.TTS_VOICE || "Rex",
    response_format: "mp3",
  };
  // Instruction de ton (rôle/émotion/débit) : supportée par certains modèles (ex. Qwen). OFF par
  // défaut — Grok Rex n'en a pas besoin ; activable via TTS_INSTRUCTIONS ou l'option instructions.
