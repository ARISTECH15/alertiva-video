...
export async function ensureAccessToken() {
  ...
  if (!j.access_token) {
    if (j.error === "invalid_grant")
      throw new Error("Jeton YouTube expiré/révoqué → reconnecte la chaîne sur alertivanews.com/admin/integrations. (Passe l'app Google en « Production » pour ne plus le refaire tous les 7 jours.)");
    throw new Error("Refresh YouTube échoué : " + JSON.stringify(j));
  }
...
/** Nombre de vidéos déjà publiées sur YouTube aujourd'hui (pour respecter le quota API). */
export async function youtubeCountToday() {
  need();
  const start = new Date().toISOString().slice(0, 10) + "T00:00:00Z";
  const rows = await sbGet(`social_posts?network=eq.youtube&published_at=gte.${start}&select=post_id`);
