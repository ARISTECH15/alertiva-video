...
/** Interroge creator_info — OBLIGATOIRE côté TikTok avant chaque Direct Post. Renvoie data. */
export async function creatorInfo(accessToken) {
  ...
  const j = await r.json();
  if (!j?.data) throw new Error("creator_info : " + JSON.stringify(j));
  return j.data;
}

/**
 * DIRECT POST : publie directement le MP4 sur le profil (@alertiva), sans passer par l'inbox.
 * Nécessite le scope `video.publish` ET une app AUDITÉE — sinon TikTok force la visibilité en
 * privé (SELF_ONLY). Renvoie publish_id.
 */
export async function postVideoDirect(buf, accessToken, { title = "", privacy = "PUBLIC_TO_EVERYONE" } = {}) {
  await creatorInfo(accessToken); // exigé par TikTok avant chaque Direct Post
  const size = buf.length;
  const MAX = 64 * 1024 * 1024;
  const single = size <= MAX;
  const chunkSize = single ? size : 32 * 1024 * 1024;
  ...
  const initJson = await initRes.json();
  if (initJson?.error && initJson.error.code && initJson.error.code !== "ok") {
    throw new Error("init direct : " + JSON.stringify(initJson.error));
  }
  const publishId = initJson?.data?.publish_id;
  const uploadUrl = initJson?.data?.upload_url;
  if (!publishId || !uploadUrl) throw new Error("init direct sans upload_url : " + JSON.stringify(initJson));
...
/** Poll jusqu'à PUBLISH_COMPLETE (Direct Post publié) ou échec. */
export async function waitPublish(publishId, accessToken, tries = 30) {
  for (let i = 0; i < tries; i++) {
    ...
    if (st === "PUBLISH_COMPLETE") return "PUBLISH_COMPLETE";
